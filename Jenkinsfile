    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep account-middleware | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/account-middleware:latest/account-middleware:$branch/g" cmd/account-middleware/k8s/02-account-middleware.yaml
