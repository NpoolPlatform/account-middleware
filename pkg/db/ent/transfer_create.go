...
func (tc *TransferCreate) defaults() error {
	...
	if _, ok := tc.mutation.UserID(); !ok {
		if transfer.DefaultUserID == nil {
			return fmt.Errorf("ent: uninitialized transfer.DefaultUserID (forgotten import ent/runtime?)")
		}
		v := transfer.DefaultUserID()
		tc.mutation.SetUserID(v)
	}
	if _, ok := tc.mutation.TargetUserID(); !ok {
		if transfer.DefaultTargetUserID == nil {
			return fmt.Errorf("ent: uninitialized transfer.DefaultTargetUserID (forgotten import ent/runtime?)")
		}
...
func (tc *TransferCreate) check() error {
	...
	if _, ok := tc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`ent: missing required field "Transfer.user_id"`)}
	}
	if _, ok := tc.mutation.TargetUserID(); !ok {
		return &ValidationError{Name: "target_user_id", err: errors.New(`ent: missing required field "Transfer.target_user_id"`)}
	}
	return nil
}
