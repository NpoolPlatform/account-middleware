...
// functions as a query rule.
type OrderBenefitQueryRuleFunc func(context.Context, *ent.OrderBenefitQuery) error

// EvalQuery return f(ctx, q).
func (f OrderBenefitQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.OrderBenefitQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.OrderBenefitQuery", q)
}

// The OrderBenefitMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type OrderBenefitMutationRuleFunc func(context.Context, *ent.OrderBenefitMutation) error

// EvalMutation calls f(ctx, m).
func (f OrderBenefitMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.OrderBenefitMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.OrderBenefitMutation", m)
}
