...
// The OrderBenefitFunc type is an adapter to allow the use of ordinary
// function as OrderBenefit mutator.
type OrderBenefitFunc func(context.Context, *ent.OrderBenefitMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OrderBenefitFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.OrderBenefitMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OrderBenefitMutation", m)
	}
	return f(ctx, mv)
}
